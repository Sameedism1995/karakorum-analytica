...
from app.services.buffer_posting_service import post_to_dict, send_post_to_buffer
...
def run_e2e_local_to_buffer(db: Session, *, dry_run: bool = True) -> dict[str, Any]:
    """Run full pipeline; dry_run=True skips Buffer send (default)."""
    ...
    buffer_result = send_post_to_buffer(db, approved.id)
    result["steps"]["buffer"] = buffer_result
    result["ok"] = bool(buffer_result.get("ok"))
    result["post"] = buffer_result.get("post") or post_to_dict(approved)
    if buffer_result.get("ok"):
        result["message"] = "Live E2E complete — post sent to Buffer queue"
        logger.info(f"E2E live complete post_id={approved.id} status=sent_to_buffer")
    else:
        result["error"] = buffer_result.get("error") or "Buffer send failed"
        logger.error(f"E2E buffer failed post_id={approved.id} error={result['error'][:120]}")
